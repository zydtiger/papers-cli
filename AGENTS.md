...
- Before release, run `uv run ruff format --check .`, `uv run ruff check .`, `uv run mypy`, `uv run pytest`, and `uv build`. Also run an isolated live-provider download and verification smoke test with temporary data and cache directories; do not add live networking to the offline test suite.
