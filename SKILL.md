...
Use source-qualified remote references such as `arxiv:2301.00001`, `biorxiv:10.1101/2024.01.01.123456`, or `pmc:PMC3531190`. Unqualified arXiv identifiers, valid bioRxiv `10.1101/...` DOIs, and PMCIDs such as `PMC3531190.1` remain supported. PMC lookup selects the current version through the official ID Converter unless a version is explicit, then reads official PMC Cloud metadata for that version. `doi:DOI` is a local collection alias only: use it only after the paper is already stored, and expect `unsupported_ref` if it cannot be resolved locally. Papers CLI has no generic remote DOI resolver, so a DOI outside bioRxiv reports `unsupported_ref`. A malformed source-qualified identifier reports `invalid_ref`; a valid remote or local identifier that is absent reports `not_found`.
