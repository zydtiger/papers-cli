...
`papers` is a local-first, cross-platform command-line tool for discovering paper metadata, downloading official full text, and keeping a verifiable local collection. It supports the official arXiv Atom API, bioRxiv API, Crossref metadata API, and PMC ID Converter plus PMC Cloud metadata.
...
Crossref DOI records retain `crossref:DOI` as their metadata identity even when the DOI maps to PMC. Files retrieved through that mapping record `pmc` as their provider and the selected PMC version, so a metadata source is never mistaken for a file source.
