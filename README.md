...
`papers` is a local-first, cross-platform command-line tool for discovering paper metadata, downloading official full text, and keeping a verifiable local collection. It supports the official arXiv Atom API, bioRxiv API, Crossref metadata API, PubMed E-utilities, and PMC ID Converter plus PMC Cloud metadata.
...
- `papers sources --jsonl` reports installed source capabilities, one record per source. `metadata_search` reports whether a provider supports keyword metadata search, `lookup` reports identifier lookup support, `reference_formats` lists supported remote identifiers, and `fulltext_formats` lists directly available full-text media formats. The CLI accepts `pdf`, `txt`, and `xml` storage requests, but installed providers determine which can be retrieved: arXiv and bioRxiv advertise PDF only; PMC advertises PDF, text, and JATS XML subject to each article version's Cloud metadata. Crossref resolves DOI metadata and PubMed resolves PMID metadata; their `delivery_source` and `delivery_formats` describe the PMC-mapped formats that can be downloaded when a record has a PMC mapping. The existing `search` and `download` fields remain for compatibility; bioRxiv reports `search: "doi_only"` because a valid bioRxiv DOI is accepted as a lookup convenience despite `metadata_search: false`.
- `papers search --source SOURCE --query QUERY --limit N --jsonl` searches official metadata. arXiv supports a general query. PubMed uses ESearch followed by one batched ESummary request, returning reusable `pubmed:PMID` records in ESearch order. It does not make one PMC mapping request per discovery result, so search records report `fulltext_availability: "unknown"` even when their metadata includes a PMCID. bioRxiv accepts only a valid `10.1101/...` DOI, delegates it to lookup, and reports `unsupported_search` for keywords; a DOI for another publisher reports `invalid_ref`. Add `--fields` to return only selected top-level fields per result.
- `papers lookup REF... --jsonl` resolves each reference in input order, using a local UUID/alias or a recognized remote identifier, without changing local storage. Remote identifiers are source-qualified, such as `arxiv:2301.00001`, `biorxiv:10.1101/2024.01.01.123456`, `pmc:PMC3531190`, `crossref:10.1093/nar/gks1195`, and `pubmed:23193287`; unqualified arXiv identifiers, valid bioRxiv `10.1101/...` DOIs, PMCIDs such as `PMC3531190.1`, bare DOIs, `doi:DOI`, `https://doi.org/DOI`, and `pmid:PMID` remain supported. A PubMed lookup uses ESummary for title, authors, date, identifiers, and an abstract when provided, then uses the PMC ID Converter and Cloud metadata only to discover official PMC full text. A PubMed record without a PMC mapping still returns its metadata with `fulltext_availability: "unavailable"`; a PMC mapping request that cannot complete leaves availability `"unknown"`. The emitted `pubmed:PMID` reference always selects its stored PubMed record locally, while `pmid:PMID` is a shorthand for that record. DOI references first resolve Crossref metadata, then use the PMC ID Converter and Cloud metadata only to discover official PMC full text. A DOI that is not in Crossref reports `not_found` scoped to Crossref; Crossref coverage does not establish that a DOI is globally nonexistent. A DOI with no PMC record still returns Crossref metadata with `fulltext_availability: "unavailable"`; a PMC mapping request that cannot complete leaves availability `"unknown"`. A PMC lookup uses the ID Converter to select the current version unless a version is explicit, then reads that version's official PMC Cloud metadata. A malformed source-qualified identifier reports `invalid_ref`, while a valid identifier missing locally or from its source reports `not_found`. Order and duplicate references are preserved, emitting one record per input. `--fields` projects the shared remote-paper vocabulary onto local and remote records alike.
- `papers download REF... --format pdf|txt|xml --jsonl` obtains exactly one official full-text format and persists metadata and provenance. `--format` defaults to `pdf`; Papers CLI never converts a different format or falls back to another available format. If the source does not provide the requested format, it reports `format_unavailable` with the requested and available formats. DOI and PMID downloads use only a mapped PMC Cloud object; Crossref and PubMed landing URLs are never download URLs. It follows the same remote-reference rules as `lookup`. Search and lookup results return reusable `ref` values.
...
Crossref DOI records retain `crossref:DOI` and PubMed records retain `pubmed:PMID` as their metadata identities even when they map to PMC. Files retrieved through either mapping record `pmc` as their provider and the selected PMC version, so a metadata source is never mistaken for a file source.
